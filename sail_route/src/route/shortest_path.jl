...
function route_solve(route::Route, performance, start_time::DateTime,
    ...
    for idy in 1:size(x)[1]-1
        @simd for idx in 1:size(x)[2]
            ...
            wadi_int = wadi[:interp](time=start_time, lon_b=x[idy, idx], lat_b=y[idy, idx])[:data][1]
            wahi_int = wahi[:interp](time=start_time, lon_b=x[idy, idx], lat_b=y[idy, idx])[:data][1]
    ...
    for idx in 1:size(x)[2]
        ...
        t = start_time + convert_time(earliest_times[end, idx])
         ...
         wadi_int = wadi[:interp](time=start_time, lon_b=x[end, idx], lat_b=y[end, idx])[:data][1]
         wahi_int = wahi[:interp](time=start_time, lon_b=x[end, idx], lat_b=y[end, idx])[:data][1]
         @inbounds speed = cost_function(performance, cd_int, cs_int,
                                         wd_int, ws_int, wadi_int, wahi_int, b)
        if speed != Inf 
